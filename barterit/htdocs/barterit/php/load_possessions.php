...
if (isset($_POST['userid'])) {
    ...
    if (isset($_POST['publish']) && $_POST['publish'] == 'true') {
        $sqlloadpossessions .= " AND publish = 1 AND available = 1";
    } elseif (isset($_POST['publish']) && $_POST['publish'] == 'false') {
        $sqlloadpossessions .= " AND publish = 0 AND available = 1";
    }
}
// Search
...
// Buyer tab screen
    ...
    if (isset($_POST['publish']) && $_POST['publish'] == 'true') {
        $sqlloadpossessions .= " WHERE publish = 1 AND available = 1";
    }
}
...
$sqlloadpossessions = $sqlloadpossessions . " ORDER BY possession_id DESC LIMIT $page_first_result, $results_per_page";
$result = $conn->query($sqlloadpossessions);
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $possessionlist['cash_checked'] = (bool)$row['cash_checked'];
        $possessionlist['goods_checked'] = (bool)$row['goods_checked'];
        $possessionlist['services_checked'] = (bool)$row['services_checked'];
        $possessionlist['other_checked'] = (bool)$row['other_checked'];
        $possessionlist['publish'] = (bool)$row['publish'];
        $possessionlist['available'] = (bool)$row['available'];
